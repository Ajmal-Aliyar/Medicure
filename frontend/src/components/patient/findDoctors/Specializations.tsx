            ...
            <div className='overflow-x-auto w-full p-8'>
                <div className='flex gap-8 min-w-[600px]'>
                    {specializationsData.map((specialization, index) => (
                        <div
                            key={index}
                            className=' cursor-pointer active:scale-90 duration-300 w-[120px] min-w-[120px] aspect-square rounded-full bg-[#51aff6ce] p-3 relative flex justify-center items-center'
                        >
                            <div className='absolute w-[90px] h-20 bg-[#c3d6e4] blur-lg rounded-full'></div>
                            <img
                                src={specialization.image}
                                alt={specialization.name}
                                className='z-10 rounded-full hover:scale-110 duration-300'
                            />
                            <p className='absolute -bottom-8 text-lg font-medium'>{specialization.name}</p>
                        </div>
                    ))}
                </div>
            </div>
