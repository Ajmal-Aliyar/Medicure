...
import { ISpecializationDocument, ISpecializationRepository } from "../interfaces/ISpecializationRepository";


export class SpecializationRepository implements ISpecializationRepository {

    async createSpecialization(name: string, image: string, description: string): Promise<ISpecializationDocument> {
        try {
            const specialization = new SpecializationModal({ name, image, description })
