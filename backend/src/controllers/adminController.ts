...
export class AdminController {
  ...
  constructor(adminServices: IAdminServices) {
    ...
    this.getDoctorAppointmentDetails =
      this.getDoctorAppointmentDetails.bind(this);
  ...
  async getApprovedDoctors(
    ...
    try {
      ...
      const approvedDoctors = await this.adminServices.getApprovedDoctors(
        skip,
        limit,
        searchQuery
      );
  ...
  async getDoctorApprovalRequests(
    ...
    try {
      ...
      const approvedDoctors =
        await this.adminServices.getDoctorApprovalRequests(skip, limit);
      res.status(200).json({
        success: true,
        data: approvedDoctors.data,
  ...
  async getDoctorAppointmentDetails(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      const DoctorSlots = await this.adminServices.getDoctorAppointmentDetails(
        _id
      );
