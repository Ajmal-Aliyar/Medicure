...
export class WithdrawController {
    ...
    async createWithdrawRequest(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { role } = req.client
            req.body.role = role
            const response = await this.withdrawService.createWithdrawRequest(req.body)
    ...
    async getWithdrawRequests( req: Request, res: Response, next: NextFunction ): Promise<void> {
        try {
            const status = req.query.status as string || '';
            const skip = parseInt(req.query.skip as string) || 0;
            const limit = parseInt(req.query.limit as string) || 5;

            const response = await this.withdrawService.getWithdrawRequests( status, skip, limit)
    ...
    async getWithdrawRequestsByUser( req: Request, res: Response, next: NextFunction ): Promise<void> {
        try {
            console.log('sdfhj sdfj ldfsj kjfds');
            
            const { _id } = req.client
            console.log(_id,'sdfdfsfdsddsfd sdfsdf');
            
            const status = req.query.status as string || '';
            const skip = parseInt(req.query.skip as string) || 0;
            const limit = parseInt(req.query.limit as string) || 5;

            const response = await this.withdrawService.getWithdrawRequestsByUser( _id, status, skip, limit)
            res.status(200).json(response)
        } catch (error: unknown) {
            next(error)
        }
    ...
    async cancelWithdrawRequest( req: Request, res: Response, next: NextFunction ): Promise<void> {
        try {
            const id = req.query.id as string
            console.log(id,req.query);
