...
export class WithdrawService implements IWithdrawService {
    ...
    constructor(withdrawRepository: IWithdrawRepository, walletRepository: IWalletRepository, transactionRepository: ITransactionRepository) {
    ...
    async getWithdrawRequests(
        status: string, skip: number, limit: number): Promise<{ withdrawRequests: IWithdrawRequests[], total: number }> {
        try {
            return await this.withdrawRepository.getWithdrawRequests(status, skip, limit)
        } catch (error: unknown) {
            console.error(error)
            throw error
        }
    ...
    async getWithdrawRequestsByUser(
        ...
        try {
            return await this.withdrawRepository.getWithdrawRequests( status, skip, limit, _id)
        } catch (error: unknown) {
            console.error(error)
            throw error
        }
    ...
    async approveWithdrawRequest(
        ...
        try {
            const companyWallet = await this.walletRepository.getWalletBalanceByOwnerId('Company')
            const request: IWithdrawSchema = await this.withdrawRepository.getWithdrawRequestById(_id)

            if (companyWallet < request.amount) {
                throw new Error ('Insufficient balance in accound.')
            }

            const updateResult = await this.withdrawRepository.updateWithdrawRequest(_id, 'approved')
    ...
    async cancelWithdrawRequest(
        ...
        try {
            const updateResult = await this.withdrawRepository.updateWithdrawRequest(_id, 'rejected')
