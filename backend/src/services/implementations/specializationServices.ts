...
export class SpecializationServices implements ISpecializationServices {
  ...
  async createSpecialization({
    ...
    try {
      const response = await this.specializationRepository.createSpecialization(
        name,
        image,
        description
      );
      ...
      throw `Failed to create specialization: ${error}`;
    }
  }
  ...
  async fetchAllSpecialization(): Promise<ISpecialization[]> {
    try {
      ...
      throw `Failed to fetch specializations: ${error}`;
    }
  }
  ...
  async fetchByName(name: string): Promise<ISpecialization> {
    try {
      ...
      throw `Failed to fetch specialization: ${error}`;
    }
  }
}
