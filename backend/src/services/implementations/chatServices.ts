...
export class ChatServices implements IChatServices {
  ...
  async getUserChats({
    ...
    try {
      ...
      const client =
        role === "doctor" ? this.patientRepository : this.doctorRepository;

      const populatedChats = await Promise.all(
        chats.map(async (chat) => {
          // Convert Map to Object for easier access on frontend
          const unreadMessagesObject = Object.fromEntries(
            chat.unreadMessages || []
          );
  ...
  async updateLastMessage({
    chatId,
    messageId,
  }: IUpdateLastMessage): Promise<IChat> {
    try {
      const updatedChat = await this.chatRepository.updateLastMessage(
        chatId,
        messageId
      );
