...
export class PaymentServices implements IPaymentServices {
  ...
  constructor(
    transactionServices: ITransactionServices,
    appointmentServices: IAppointmentServices,
    slotServices: ISlotService,
  ...
  async checkoutSession(data: ICheckoutSession) {
    try {
      const isAvailable = await this.slotServices.isSlotLimitExceed(
        data.slotId
      );
