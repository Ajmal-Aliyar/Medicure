...
export class DoctorMedicalRecordController implements IDoctorMedicalRecordController {
    constructor(@inject(TYPES.DoctorMedicalRecordService) private readonly medicalRecordService: IDoctorMedicalRecordService) {}

    getPatientMedicalRecordsByAppointmentId = async(req: Request, res: Response): Promise<void> => {
        const { appointmentId } = req.params
        const { id } = req.user;
        const pagination = getPaginationParams(req)
        const { data, total } = await this.medicalRecordService.getReportsByAppointmentId( id, appointmentId, pagination)
